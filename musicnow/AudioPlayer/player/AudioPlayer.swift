...
public class AudioPlayer: NSObject {
        ...
        didSet {
            if let currentItem = currentItem {
                ...
                if oldValue != currentItem {
                    self.delegate?.audioPlayer(self, updatePlaying: currentItem)
                }
                
                
                currentItem.url { (info) in
                ...
                //let info = currentItem.url(currentQuality)
    ...
    /// is `multiplyRate(2)`.
    public var seekingBehavior = SeekingBehavior.multiplyRate(1) {
        didSet {
