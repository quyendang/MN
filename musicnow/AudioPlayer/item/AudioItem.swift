...
//    }
    ...
    public init?(videoId: String, title: String?, artist: String?, imageUrl: String?) {
        self.videoId = videoId
        self.title = title
        self.artist = artist
        self.artworkUrl = imageUrl?.replacingOccurrences(of: "hqdefault", with: "maxresdefault")
...
//    }
    ...
    func url(_ completed: @escaping (_ itemUrl: AudioItemURL) -> Void) {
        if let videoId = self.videoId {
            XCDYouTubeClient.default().getVideoWithIdentifier(videoId) { (v, error) in
                guard let video = v else {return}
                
                guard let u = video.streamURLs[18] else {return}
                self.mediumQualityURL = AudioItemURL(quality: .medium, url: u)!
                completed(AudioItemURL(quality: .medium, url: u)!)
            }
        }
    }
