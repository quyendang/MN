...
extension UIImage {
    ...
    public func getColorsAddFillter(index: Int, quality: UIImageColorsQuality = .high, _ completion: @escaping (UIImage?) -> Void) {
        DispatchQueue.global().async {
            let ix = (index+1) % 15
            //let result = self.getColors(quality: quality)
            //let output = self.addFillterColor((CIColor(color: result!.primary), CIColor(red: 0.09, green: 0.14, blue: 0.32, alpha: 1.0)))
            let output = self.addFillterColor((CIColor(color: UIColor(named: "1l-\(ix)")!), CIColor(color: UIColor(named: "1d-\(ix)")!)))
            DispatchQueue.main.async {
                completion(output)
            }
        }
    ...
    func addFillterColor(_ color: ColorFillter) -> UIImage {
        //let _lightColor = CIColor(red: 0.15, green: 0.89, blue: 0.47, alpha: 1.0)
        //let _darkColor = CIColor(red: 0.09, green: 0.14, blue: 0.32, alpha: 1.0)
        //let _lightColor = CIColor(color: lightColor)
        //let _darkColor = CIColor(color: darkColor)
        let grayscaleFilter = CIFilter(name: "CIPhotoEffectMono")
        grayscaleFilter?.setValue(CIImage(cgImage: self.cgImage!), forKey: kCIInputImageKey)
        
        let multiplyFilter = CIFilter(name: "CIMultiplyBlendMode")
        let multiColorImage = CIImage(color: color.lightColor)
        multiplyFilter?.setValue(grayscaleFilter?.outputImage, forKey: kCIInputImageKey)
        multiplyFilter?.setValue(multiColorImage, forKey: kCIInputBackgroundImageKey)
        
        let lightenFilter = CIFilter(name: "CILightenBlendMode")
        let colorImage = CIImage(color: color.darkColor)
        lightenFilter?.setValue(multiplyFilter?.outputImage, forKey: kCIInputImageKey)
        lightenFilter?.setValue(colorImage, forKey: kCIInputBackgroundImageKey)
        
        if let output = lightenFilter?.outputImage, let groutput = grayscaleFilter?.outputImage {
            let context:CIContext = CIContext.init(options: nil)
            let cgImage:CGImage = context.createCGImage(output, from: groutput.extent)!
